...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
BOOL XYSocketsStartup(PXYSOCKET ps, LPVOID parameter0, LPVOID parameter1, XYSOCKET_PROCEDURE procedure);
VOID XYSocketsCleanup(PXYSOCKET ps);

PXYSOCKET_SET XYSocketLaunchThread(PXYSOCKET ps, UINT thread_index, UINT maximum);

SOCKET XYUDPBind(PXYSOCKET ps, int type, LPVOID *pointer, LPVOID context, const SOCKADDR *psa, int addresslength);
int XYUDPSendTo(SOCKET s, const SOCKADDR *psa, int addresslength, const char *buffer, int length);

SOCKET XYTCPConnect(PXYSOCKET ps, LPVOID context, const SOCKADDR *psa, int addresslength, int sendbuffersize);
SOCKET XYTCPListen(PXYSOCKET ps, LPVOID *pointer, LPVOID context, const SOCKADDR *psa, int addresslength);
int XYTCPSend(SOCKET s, const char *buffer, int length, UINT seconds);
//---------------------------------------------------------------------------
#endif